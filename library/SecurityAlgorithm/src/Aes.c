		...
		int p_chiperSize, char *key)
{
    ...
    if(p_plainSize == 0 || p_plainSize % 16 != 0 || p_plainText == NULL)
    {
        printf("Plaintext characters must be a multiple of 16, Current length is %d\n",
        		p_plainSize);
        return -1;
    }
    if(p_chiperSize < p_plainSize || p_chiperText == NULL)
    {
    	printf("The ciphertext space must be larger than the plaintext space.");
    	return -1;
    }

    if(!checkKeyLen(keylen))
    {
        printf("The key character length must be 16, 24 and 32. Current length is %d\n",
        		keylen);
        return -1;
    }
		...
		int p_plainSize, char* key)
{
    ...
    {
        convertToIntArray(p_chiperText + k, cArray);
        ...
        convertArrayToStr(cArray, p_plainText + k);
    }
    return 0;
}
